...
Future versions might contain code for GANs too.
"""
...
def build(params):
    ...
    # Conv block
    conv_1 = Conv1D(filters = params['conv_filters'][0], kernel_size = params['kernel_size'], activation = params['conv_activation'],
                    kernel_initializer = params['conv_initializer'], padding = 'same', name = 'conv1')(encoder_input)
    if params['use_batchnorm']:
        conv_1 = BatchNormalization(name = 'batchnorm_1')(conv_1)
    conv_2 = Conv1D(filters = params['conv_filters'][1], kernel_size = params['kernel_size'], activation = params['conv_activation'],
                    kernel_initializer = params['conv_initializer'], padding = 'same', name = 'conv2')(conv_1)
    if params['use_batchnorm']:
    ...
    decoder_lstm = LSTM(params['decoder_lstm_units'], return_sequences = True, name = 'decoder_lstm')(concatenation)
    decoder_dense = TimeDistributed(Dense(params['decoder_dense_units'], activation = params['decoder_dense_activation']), name = 'decoder_dense')(decoder_lstm)
    decoder_output = TimeDistributed(Dense(1, activation = params['decoder_output_activation']), name = 'decoder_output')(decoder_dense)
